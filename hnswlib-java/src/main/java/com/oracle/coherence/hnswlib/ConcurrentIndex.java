...
public class ConcurrentIndex
     ...
     * Internally, an incremental ID (starting from 1) will be given to this
     * item.
     *
    ...
    /**
     * Thread-safe method which adds an item with ID to the index. It won't
     * apply any extra normalization unless it is required by the Vector Space
     * (e.g., COSINE).
     *
     * @param item - float array with the length expected by the index
     *             (dimension);
     ...
     * Internally, an incremental ID (starting from 0) will be given to this
     * item.
     *
     * @param item - float array with the length expected by the index
     *             (dimension).
    ...
    public void addNormalizedItem(float[] item)
        ...
        try
            {
            super.addNormalizedItem(item, NO_ID);
            }
        finally
            {
            this.writeLock.unlock();
    ...
    /**
     * Thread-safe method which returns the number of elements already inserted
     * in the index.
     *
    ...
    /**
     * Thread-safe method which performs a knn query in the index instance. In
     * case the vector space requires the input to be normalized, it will
    ...
    /**
     * Thread-safe method which performs a knn query in the index instance using
     * an normalized input. It will not normalize the vector again.
     *
    ...
    /**
     * Thread-safe method which loads the content stored in a file path onto the
     * index.
     * <p>
    ...
    /**
     * Thread-safe method which frees the memory allocated for this index in the
     * native context.
     * <p>
    ...
    /**
     * Thread-safe method which sets the query time accuracy / speed trade-off
     * value.
     *
     * @param ef value.
     */
    ...
    /**
     * Thread-safe method that checks whether there is an item with the
     * specified identifier in the index.
     *
     * @param id - identifier.
     *
    ...
    /**
     * Thread-safe method that gets the data from a specific identifier in the
     * index.
     *
     * @param id - identifier.
     *
